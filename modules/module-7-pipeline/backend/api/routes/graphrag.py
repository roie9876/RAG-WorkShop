...
# ────────────────────────────────────────────────────────────
# KG Search Index — fast vector search over GraphRAG output
# ────────────────────────────────────────────────────────────

@router.get("/kg-index/status")
async def get_kg_index_status():
    """
    Get the status of the KG Search index (fast GraphRAG via AI Search).
    
    Returns document counts for entity profiles and community summaries.
    """
    try:
        settings = get_settings()
        from services.kg_search_service import KGSearchIndexService
        service = KGSearchIndexService(settings.graphrag_index_path)
        status = service.get_status()
        return {"success": True, "status": status}
    except Exception as e:
        logger.error(f"Failed to get KG index status: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/kg-index/build")
async def build_kg_index(force_recreate: bool = True):
    """
    Build the KG Search index from GraphRAG Parquet output.
    
    This converts entity profiles and community summaries into an
    Azure AI Search index for fast vector search (~2-5s vs ~40-120s).
    
    Process:
    1. Load entities (degree ≥ 3) and community reports from Parquet
    2. Build entity profiles with relationship context
    3. Generate embeddings (text-embedding-3-large)
    4. Upload to Azure AI Search index
    
    Typically takes 2-5 minutes for ~1000 documents.
    """
    try:
        settings = get_settings()
        from services.kg_search_service import KGSearchIndexService
        ...
        logger.info("Starting KG Search index build...")
        result = await service.build_index(force_recreate=force_recreate)
        ...
        logger.error(f"KG index build failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.delete("/kg-index")
async def delete_kg_index():
    ...
    try:
        ...
        return {"success": True, "message": "KG Search index deleted"}
    except Exception as e:
        logger.error(f"Failed to delete KG index: {e}")
        raise HTTPException(status_code=500, detail=str(e))
