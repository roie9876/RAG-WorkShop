...
התשובות משלבות מידע שמגיע מ**שלושה סוגי מקורות** שונים, שהפייפליין מעבד ומאנדקס יחד:

#### 1. מפת קווי המטרו המתוכננים לגוש דן

<div align="center">
<img src="../../data/sample-pdfs/m1.png" alt="מפת קו M1 – מטרו גוש דן" width="600"/>
<br/><em>מפת קו M1 של מטרו גוש דן – כולל תחנות, זרועות ומחלפים</em>
</div>

> 📍 קובץ: [`data/sample-pdfs/m1.png`](../../data/sample-pdfs/m1.png)

#### 2. קובץ נתונים טבלאי – פירוט תחנות קו M1

קובץ Excel עם מידע מפורט על כל תחנה בקו M1 בראשון לציון, כולל שם התחנה ותיאור מקיף (מיקום, תכנון סביבתי, השפעה על מקרקעין, חיבורים תחבורתיים):

| מספר | שם תחנה | סוג מידע |
|------|---------|----------|
| 1 | בנות חיל | מיקום, השפעות מקרקעין, חיבורים, מועד ביצוע |
| 2 | ראשון לציון מרכז | ציר ז'בוטינסקי-הרצל, התחדשות עירונית |
| 3 | הראשונים | מחלף רכבת ישראל + BRT, דיפו |
| 4 | שדרות הציונות | כניסות ומפלסים, קישוריות |
| 5 | יוסף בורג | תכנון שכונתי, ייעודי קרקע |
| 6 | הרצוג | תכנית פיתוח רחבה, יח"ד |
| 7 | אסף הרופא | קרבה למרכז רפואי |
| 8 | צריפין | מתחם מתפנה, שכונה חדשה |

> 📍 קובץ: [`data/sample-office/metro_m1_data.xlsx`](../../data/sample-office/metro_m1_data.xlsx)

#### 3. מקבץ דוחות PDF – תחנות 35–41 בזרוע המזרחית

דוחות תכנון מפורטים לכל תחנה בנפרד, הכוללים מפות, חתכים, ייעודי קרקע, תשתיות ותכניות מאושרות:

| קובץ | תחנה |
|------|------|
| `metro-s35.pdf` | תחנה 35 – קפלן |
| `metro-s36.pdf` | תחנה 36 – שדרות הציונות |
| `metro-s37.pdf` | תחנה 37 – יוסף בורג |
| `metro-s38.pdf` | תחנה 38 – הרצוג |
| `metro-s39.pdf` | תחנה 39 – אסף הרופא |
| `metro-s40.pdf` | תחנה 40 – צריפין |
| `metro-s41.pdf` | תחנה 41 – באר יעקב |
| `m1s-s35-s41.pdf` | סקירה כוללת תחנות 35–41 |

> 📍 נתיב: [`data/sample-pdfs/`](../../data/sample-pdfs/)

---

### 💡 למה צריך Combined?

כל מקור לבד נותן תמונה חלקית:
- **AI Search** מוצא פרטים ספציפיים מתוך דוחות ה-PDF (כניסות, מפלסים, תכניות) ומהאקסל (תיאורי תחנות).
- **GraphRAG** מוצא **קשרים בין ישויות** (תחנות סמוכות, השתייכות לרשויות, ארגונים מתכננים) על סמך גרף הידע שנבנה מכל המסמכים.
- **השילוב** נותן תמונה מלאה: גם פרטי עומק מהמסמכים וגם הבנת הקשרים בין הישויות.
